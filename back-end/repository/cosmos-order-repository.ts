...
export class CosmosOrderRepository {
    ...
    private async toOrder(doc: CosmosOrderDocument): Promise<Order> {
        ...
        const customer = await customerRepo.getCustomerById(parseInt(doc.customerId));
        if (!customer) {
    ...
    async getOrderById(id: number): Promise<Order | undefined> {
        ...
        const { resource } = await this.container.item(idStr, partition).read<CosmosOrderDocument>();
        if (!resource) {
            return undefined;
        }
        ...
        try {
            ...
            for (const item of items) {
                ...
                if (currentProduct.getStock() < item.getQuantity()) {
                    throw CustomError.invalid(`Insufficient stock for product ${product.getName()}`);
                }
