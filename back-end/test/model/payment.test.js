...
let payment;
beforeEach(() => {
    payment = new payment_1.Payment(validPaymentTestData);
});
const { amount, date, paymentStatus } = validPaymentTestData;
const createPayment = (overrides = {}) => new payment_1.Payment(Object.assign(Object.assign({}, validPaymentTestData), overrides));
...
test('given: invalid paymentStatus for payment, when: payment is created, then: an error is thrown.', () => {
    expect(() => createPayment({ paymentStatus: 'false' })).toThrow('Payment status must be either paid or unpaid.');
});
test('given: invalid date format for payment, when: payment is created, then: an error is thrown.', () => {
    expect(() => createPayment({ date: new Date('invalid-date-string') })).toThrow('Invalid date provided.');
});
test('given: future date for payment, when: payment is created, then: an error is thrown.', () => {
    expect(() => createPayment({ date: (0, date_fns_1.set)(new Date(), { year: 2124, month: 2, date: 23 }) })).toThrow('Payment date cannot be in the future.');
});
