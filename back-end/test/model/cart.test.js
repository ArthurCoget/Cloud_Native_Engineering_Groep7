...
let fixedDiscountCode1;
let fixedDiscountCode2;
beforeEach(() => {
    customer = new customer_1.Customer(Object.assign(Object.assign({}, customerTestData), { wishlist: [] }));
    product = new product_1.Product(productTestData);
    cart = new cart_1.Cart({ customer, products: [], discountCodes: [] });
    percentageDiscountCode = new discountCode_1.DiscountCode(percentageDiscountTestData);
    fixedDiscountCode1 = new discountCode_1.DiscountCode(fixedDiscountTestData1);
    fixedDiscountCode2 = new discountCode_1.DiscountCode(fixedDiscountTestData2);
});
...
test('given: invalid customer, when: cart is created, then: an error is thrown.', () => {
    expect(() => new cart_1.Cart({ customer: null, products: [], discountCodes: [] })).toThrow('Customer cannot be null or undefined.');
});
...
test('given: product in cart, when: removing more items than there are in the cart, then: error is thrown.', () => {
    cart.addItem(product, 2);
    expect(() => cart.removeItem(product, 5)).toThrow('There are not that many products in the cart to remove.');
});
...
test('given: percentage discount already applied, when: another percentage discount is applied, then: error is thrown.', () => {
    cart.applyDiscountCode(percentageDiscountCode);
    expect(() => cart.applyDiscountCode(percentageDiscountCode)).toThrow('Only one percentage discount can be applied.');
});
test('given: inactive discount code, when: applied, then: error is thrown.', () => {
    const inactiveDiscountCode = new discountCode_1.DiscountCode(Object.assign(Object.assign({}, fixedDiscountTestData1), { isActive: false }));
    expect(() => cart.applyDiscountCode(inactiveDiscountCode)).toThrow('Inactive or expired discount code.');
});
...
test('given: discount code not in cart, when: attempting to remove discount code, then: error is thrown.', () => {
    cart.addItem(product, 5);
    cart.applyDiscountCode(fixedDiscountCode1);
    expect(() => cart.removeDiscountCode(percentageDiscountCode)).toThrow('That discount code had not been applied to your cart.');
});
...
test('given: discount code already applied, when: same discount code is applied again, then: error is thrown.', () => {
    cart.addItem(product, 5);
    cart.applyDiscountCode(fixedDiscountCode1);
    expect(() => cart.applyDiscountCode(fixedDiscountCode1)).toThrow('This discount code has already been applied to your cart.');
});
