...
export class Product {
  ...
  getAverageRating(): number {
    ...
    const totalRating = this.reviews.reduce(
      (sum, review) => sum + review.getRating(),
      0
    );
  ...
  getReviewByCustomerId(customerId: string): DomainReview | undefined {
  ...
  public validate(product: {
    ...
    if (
      !product.sizes.every(
        (size) => ALLOWED_SIZES.includes(size) || /^\d+$/.test(size)
      )
    ) {
      throw new Error(
        `Invalid sizes: ${product.sizes.join(
          ", "
        )}. Allowed: ${ALLOWED_SIZES.join(", ")} or numbers.`
      );
    }
  ...
  static fromCosmos(doc: CosmosProductDocument): Product {
    return new Product({
      ...
      reviews: (doc.reviews ?? [])
        .map((r) =>
          DomainReview.fromCosmos({ ...r, comment: r.comment ?? null })
        )
