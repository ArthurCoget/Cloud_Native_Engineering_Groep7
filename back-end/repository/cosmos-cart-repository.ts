...
export class CosmosCartRepository {
    ...
    private async toCart(doc: CosmosCartDocument): Promise<Cart> {
        ...
        let customer = await customerRepo.getCustomerById(parseInt(doc.customerId));
        

        if (!customer) {
            throw CustomError.notFound(`Customer with ID ${doc.customerId} not found`);
        }
        ...
        const products = await Promise.all(
            doc.products.map(async (item) => {
                ...
                if (!product) {
                    throw CustomError.notFound(`Product with ID ${item.productId} not found`);
                }
        ...
        const discountCodes = await Promise.all(
            doc.discountCodes.map(async (dc) => {
                ...
                if (!discountCode) {
                    throw CustomError.notFound(`Discount code with ID ${dc.id} not found`);
                }
                return discountCode;
            })
        );
    ...
    async getCartById(id: number): Promise<Cart | null> {
        ...
        const { resource } = await this.container.item(idStr, partition).read<CosmosCartDocument>();
        if (!resource) {
            return null;
        }
    ...
    async addDiscountCode(cart: Cart, discountCode: DiscountCode): Promise<DiscountCode> {
    ...
    async removeDiscountCode(cart: Cart, code: string): Promise<string> {
        const discountCode = cart.getDiscountCodes().find(dc => dc.getCode() === code);
        if (!discountCode) {
    ...
    private async updateCart(cart: Cart): Promise<void> {
        ...
        const { resource } = await this.container.item(idStr, partition).replace(document);
        if (!resource) {
